...
const SuccessComponent = () => {
  ...
  const [dataQrCode, setDataQrCode] = useState<any>(null) // Initialize dataQrCode state
  ...
  useEffect(() => {
    ...
    // Retrieve qrBillData from localStorage
    const qrBillData = localStorage.getItem('qrBillData')
    if (qrBillData) {
      const parsedQrBillData = JSON.parse(qrBillData)
      setDataQrCode(parsedQrBillData) // Set dataQrCode state
    }
  ...
  return (
    <div>
      <Result
        ...
        subTitle={
          <>
            LetGo5 đã gửi thông tin vé thành công tới email mà bạn cung cấp lúc thanh toán <br />
      ...
      <div className='border-[2px] border-gray-200 mx-12 rounded-[10px] overflow-hidden'>
        ...
        >
          <p style={{ marginBottom: '10px' }}>
            Mã vé: <b>{dataQrCode?.code_bill}</b>
          </p>
          <p>
            <a href={dataQrCode?.bill_url}>
              <img style={{ width: '200px', height: '200px' }} src={dataQrCode?.bill_qr_code_image} alt='bill-qrcode' />
            </a>
          </p>
        </div>
        <div className='flex mt-8'>
          ...
          <div className='flex-col ml-[450px]'>
            ...
            <div className='flex py-4'>
              <p className='text-gray-400 font-medium'>Cổng thanh toán:</p>
        ...
        <div className='flex my-10 gap-10 justify-center flex-wrap'>
          {dataTicket?.map((item: any, index: number) => (
            <TicketComponent key={index} dataTicket={item} data={data} dataQr={dataQrCode?.ticket_order[index]} />
          ))}
        </div>
      </div>
    </div>
