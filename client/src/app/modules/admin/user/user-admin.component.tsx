...
const UserAdminComponent = () => {
    ...
    useEffect(() => {
        ...
        const title = ['STT', 'tên', 'email', 'số điện thoại', 'địa chỉ', 'mô tả', 'vai trò']
        if (dataUser && dataUser.length > 0) {
            Object.keys(dataUser[0]).forEach((itemKey, key = 0) => {
                if (!['id', 'password', 'location', 'user_type_id',  'created_at', 'updated_at'].includes(itemKey)) {
                    columsTemp.push({
                        title: title[key++],
                        dataIndex: itemKey,
                        key: itemKey,
            ...
            <TemplateTable
                ...
                formEdit={
                    <Fragment>
                        ...
                        <Form.Item label='Số điện thoại' name='phone_number' rules={[{ required: true, message: 'Đây là trường bắt buộc' }]}>
                            <Input />
                        </Form.Item>
                        ...
                        <Form.Item label='Khách hàng' name='user_type_id' rules={[{ required: true, message: 'Đây là trường bắt buộc' }]}>
