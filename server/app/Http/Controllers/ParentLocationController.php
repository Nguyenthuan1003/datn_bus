...
class ParentLocationController extends Controller
{
    public function index()
    {
        ...
        try {
            ...
            ->get();
            return response()->json(['message' => 'Lấy dữ liệu thành công','parent_location' => $parentLocations], 200);
        } catch (\Exception $e) {
    ...
    public function store(Request $request)
    {
        try {
            ...
            $parentLocation->save();
            return response()->json(['message' => 'Thêm mới địa điểm thành công','parent_location' => $parentLocation], 201);
        } catch (\Exception $e) {
            return response()->json(['message' => 'Có lỗi trong quá trình thêm mới địa điểm','error' => $e->getMessage()], 501);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $parentLocation->name = $request->input('name');
            $parentLocation->save();
            return response()->json(['message' => 'Cập nhật địa điểm thành công','parent_location' => $parentLocation], 200);
        } catch (\Exception $e) {
